...
-- 2017-01-01,BLingo,Created File
--**************************************************************************--
Begin Try
	Use Master;
	If Exists(Select Name From SysDatabases Where Name = 'Assignment07DB_BLingo')
	 Begin 
	  Alter Database [Assignment07DB_BLingo] set Single_user With Rollback Immediate;
	  Drop Database Assignment07DB_BLingo;
	 End
	Create Database Assignment07DB_BLingo;
End Try
Begin Catch
	Print Error_Number();
End Catch
go
Use Assignment07DB_BLingo;
...
-- Format the date like 'January, 2017'.0
-- Order the results by the Product and Date.
-- <Put Your Code Here> --
...
SELECT
    ...
    curr.Count,
    CASE 
        WHEN MONTH(curr.InventoryDate) = 1 AND prev.Count IS NULL THEN 0
        ELSE prev.Count
    END AS PreviousMonthCount
FROM vProductInventories AS curr
LEFT JOIN vProductInventories AS prev
    ON curr.ProductName = prev.ProductName
    AND YEAR(prev.InventoryDate) = YEAR(DATEADD(MONTH, -1, curr.InventoryDate))
    AND MONTH(prev.InventoryDate) = MONTH(DATEADD(MONTH, -1, curr.InventoryDate));
go
